    ...
    switch(this._state) {
      ...
      case 2:
        if(len >= head.compressedSize) {
          ...
          if(head.generalPurposeBitFlag === 8) {
            throw new Error('ZIP Format Not Supported');
          } else {
...
Unzip.prototype.emitEntry = function(entry) {
  ...
  this.emit('entry', this._entry.pipe(zlib.createInflateRaw()), this._header);
  this._entry = null;
  this._header = null;
  this._state = 0;
  //this._rawHeader = null;
