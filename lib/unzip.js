  ...
  if(buf[0] === C.P && buf[1] === C.K) {
    switch(self._state) {
      ...
      case C.WAITING_FOR_LOCAL_FILE_HEADER:
        if(len >= info.min) {
          head = self._header = self.parseHeader(buf);
          if(head) {
      ...
      case C.WAITING_FOR_DATA_DESCRIPTOR:
        ...
        if(len >= info.min) {
          head = self._header = Object.assign({}, self._header, self.parseHeader(buf));

          self._entry = new BufferList();
          self.emitEntry(self._buffer.slice(i-head.compressedSize, i));
...
Unzip.prototype.parseHeader = function(buf) {
  ...
  var values = Object.keys(head).map(function(key) {
    var action = head[key];
