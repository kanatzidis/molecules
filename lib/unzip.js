    ...
    switch(self._state) {
      ...
      case C.WAITING_FOR_DATA_DESCRIPTOR:
        ...
        if(len >= info.min) {
          ...
          self._entry = new BufferList();
          self.emitEntry(self._buffer.slice(ignore, head.compressedSize));
          self._buffer = self._buffer.slice(ignore + info.min);
